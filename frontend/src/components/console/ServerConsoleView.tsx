...
import { Terminal, Trash2, SendHorizontal, Zap, Code, ArrowLeft } from "lucide-react";
...
export default function ServerConsoleView() {
  ...
  const [logs, setLogs] = useState<ConsoleLog[]>([
    { id: "init-1", type: "system", content: "Server Context initialized.", timestamp: new Date() },
    { id: "init-2", type: "system", content: "AI Sub-engine connected.", timestamp: new Date() },
    {
      id: "banner",
      type: "output",
      content: `Server Console — Scoped to Server #${serverId}. Type /help for available commands`,
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSubmit = async () => {
    ...
    if (!text || isLoading || !serverId) return;
    ...
    try {
      const { data: response } = await serverConsoleApi.execute(parseInt(serverId, 10), text);
    ...
    <div className="flex-1 bg-[#1e1f22] flex flex-col h-full overflow-hidden font-mono">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-1 text-[13px] selection:bg-[#5865F2]/30 scrollbar-hide">
        <div className="text-gray-500 mb-4 border-b border-[#2b2d31] pb-2">
          [SYSTEM] Server Context initialized.<br />
          [SYSTEM] AI Sub-engine connected.<br />
          [SYSTEM] Scoped to current server.
        </div>
      ...
      <footer className="p-6 bg-[#2b2d31] border-t border-[#1e1f22]">
        <div className="max-w-4xl mx-auto space-y-4">
          <div className="flex items-center gap-2 text-[#949ba4] text-xs font-bold uppercase tracking-widest px-1">
            ...
            <span>Smart Capture (Server Scoped)</span>
          </div>
