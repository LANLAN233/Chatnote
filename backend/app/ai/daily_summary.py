    ...
    if operation_id is not None:
        await manager.broadcast_ai_progress(
            ...
            stage_data=AiProgressStage(
                ...
                model="system",
                tier="system",
                message=f"已获取 {len(notes)} 条笔记 ({target_date.isoformat()})",
            ...
            if operation_id is not None:
                await manager.broadcast_ai_progress(
                    ...
                    stage_data=AiProgressStage(
                        ...
                        model=fast_model.id,
                        tier="fast",
                        message=f"正在用 {fast_model.id} 提取知识要点...",
            ...
            if operation_id is not None:
                await manager.broadcast_ai_progress(
                    ...
                    stage_data=AiProgressStage(
                        ...
                        model=fast_model.id,
                        tier="fast",
                        message=f"知识提取完成 — 发现 {len(extracted.concepts) if extracted else 0} 个概念",
                        duration_ms=extraction_duration_ms,
                        progress_pct=100,
            ...
            if operation_id is not None:
                await manager.broadcast_ai_progress(
                    ...
                    stage_data=AiProgressStage(
                        ...
                        tier="strong",
                        message=f"正在用 {strong_model.id} 生成每日总结...",
            ...
            if operation_id is not None:
                await manager.broadcast_ai_progress(
                    ...
                    stage_data=AiProgressStage(
                        ...
                        model=strong_model.id,
                        tier="strong",
                        message=f"每日总结生成完成 ({len(summary_result.summary) if summary_result else 0} 字符)",
                        duration_ms=summary_duration_ms,
                        progress_pct=100,
                    ),
                )
            ...
            if operation_id is not None:
                await manager.broadcast_ai_progress(
                    ...
                    stage_data=AiProgressStage(
                        ...
                        tier="fast",
                        message=f"正在用 {fast_model2.id} 提取关键词...",
            ...
            if operation_id is not None:
                await manager.broadcast_ai_progress(
                    ...
                    stage_data=AiProgressStage(
                        ...
                        model=fast_model2.id,
                        tier="fast",
                        message=f"关键词提取完成 — 发现 {len(keyword_mapping.keywords) if keyword_mapping else 0} 个关键词",
                        duration_ms=keywords_duration_ms,
                        progress_pct=100,
                    ),
                )
